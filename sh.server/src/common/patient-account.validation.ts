/**
 * Validation đăng ký bệnh nhân (PB01): email, SĐT VN, mật khẩu.
 */

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

/** SĐT di động VN sau chuẩn hóa: 0 + (3|5|7|8|9) + 8 chữ số */
...
/** Chuẩn hóa SĐT di động VN → đúng 10 chữ số (`0xxxxxxxxxx`). Đồng bộ với sh.client patient-register.validation. */
export function normalizeVnPhone(input: string): string | null {
  if (!input?.trim()) return null;
  let s = input.replace(/[\s.\-()_/]/g, '').trim();
  if (!s) return null;
  if (s.startsWith('+84')) {
...
/**
 * Validate input cho bệnh nhân tự cập nhật hồ sơ (PATCH /auth/me).
 * Chỉ cho phép sửa fullName + phone; email/giữ làm định danh đăng nhập, không sửa.
 */
export function validateUpdateProfileInput(input: {
  ...
  const phoneNorm = normalizeVnPhone(input.phone ?? '');
  if (!input.phone?.trim()) {
...
export function validatePatientRegisterInput(input: {
  fullName: string;
  email: string;
  phone: string;
  password: string;
