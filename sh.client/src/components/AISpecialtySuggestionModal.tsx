...
export const AISpecialtySuggestionModal: React.FC<Props> = ({
  ...
  const [acceptedAiWarning, setAcceptedAiWarning] = useState(false);
  const [result, setResult] = useState<ApiSpecialtySuggestion | null>(null);

  const reset = () => {
    setSymptoms("");
    setAcceptedAiWarning(false);
    setResult(null);
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (text.length < 10) {
      toast.error(
        "Vui lòng mô tả triệu chứng chi tiết hơn (tối thiểu 10 ký tự).",
      );
      return;
    }
    if (!acceptedAiWarning) {
      toast.error(
        "Vui lòng xác nhận rằng AI chỉ là gợi ý tham khảo trước khi tiếp tục.",
      );
      return;
    }
    setLoading(true);
  ...
  return (
    <ModalOverlay open={open} onClose={handleClose}>
      <div className="relative w-full max-w-xl bg-white rounded-3xl shadow-xl overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-4 bg-linear-to-br from-violet-50 to-blue-50 border-b border-slate-100">
          ...
          <p className="text-sm text-slate-600 leading-relaxed">
            Mô tả triệu chứng bạn đang gặp phải. Trợ lý AI sẽ gợi ý 1–3 chuyên
            khoa phù hợp để bạn cân nhắc.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="flex items-start gap-2">
            <label className="flex items-start gap-2 text-sm text-slate-700">
              <input
                type="checkbox"
                checked={acceptedAiWarning}
                onChange={(e) => setAcceptedAiWarning(e.target.checked)}
                disabled={loading}
                className="mt-1 h-4 w-4 rounded border-slate-300 text-violet-600 focus:ring-violet-500"
              />
              <span className="leading-5">
                Tôi đã đọc và đồng ý rằng AI chỉ là gợi ý tham khảo, có thể sai
                và không thay thế chẩn đoán bác sĩ.
              </span>
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={
              loading || symptoms.trim().length < 10 || !acceptedAiWarning
            }
            ...
            <div className="space-y-3 pt-2 border-t border-slate-100">
                ...
                <div className="flex gap-2 p-3 rounded-xl bg-rose-50 border border-rose-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-rose-800/90 mt-0.5">
                      Vui lòng đến cơ sở y tế gần nhất hoặc gọi cấp cứu 115
                      ngay. Đừng chờ đặt lịch online.
                    </p>
                  </div>
                </div>
              ) : null}
              ...
              {result.suggestions.length === 0 ? (
                <div className="text-sm text-slate-600 text-center py-4">
                  AI chưa đưa ra gợi ý phù hợp. Hãy thử mô tả chi tiết hơn hoặc
                  chọn <span className="font-semibold">khám tổng quát</span>.
                </div>
              ) : (
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed pt-1 border-t border-slate-100">
                Đây là gợi ý tham khảo dựa trên AI, KHÔNG phải chẩn đoán y khoa.
                Bệnh nhân vẫn nên trao đổi trực tiếp với bác sĩ để được khám và
                chẩn đoán chính xác.
              </p>
            </div>
          ) : null}
